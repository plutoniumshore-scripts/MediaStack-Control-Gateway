...
Secrets and private environment information are intentionally not stored in the public installer.
...
The real `MediaStack-Control-Gateway.config.psd1` file is excluded by `.gitignore`. Do not rename or remove that rule if the repository is public.
...
The public repository contains no intended credentials, API keys, private server addresses, usernames, or other environment specific identifiers. Runtime credentials belong only in the local configuration file.

The installer applies restricted Windows ACLs to the local configuration and generated files that can contain credentials. Large reporting exports also remain local unless explicitly read through the gateway.

Before publishing a modified copy of this project, verify that the real configuration file, generated runtime files, logs, exports, screenshots, and command output have not been added to the repository.
...
Where a specific third party source is known and materially contributes to the project, an effort will be made to identify it in the relevant file or documentation. This repository is intended to share useful work in good faith, not to claim authorship of material known to originate elsewhere.

This project also installs or interacts with third party software and APIs, including OpenAI's tunnel client, Python, the Model Context Protocol Python SDK, Plex/PlexAPI, Tautulli, Sonarr, Radarr, and Lidarr. Those projects remain subject to their own licenses and terms.
